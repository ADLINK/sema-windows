...
CI2CFunct::CI2CFunct(Ccommon_bmc &cmn)
{
	m_bBMCAdr = 0x50;
	m_clsCmn = cmn;
...
EERROR CI2CFunct::Init()
{
	...
	if (m_clsI2CTrans.Init(m_clsCmn.m_hDrv) != EAPI_STATUS_SUCCESS)
	{
...
uint32_t CI2CFunct::GetCaps()
{
	return m_nSemaCaps;
}

uint32_t CI2CFunct::GetCapsExt()
{
	return m_nSemaCapsEx;
}


uint32_t CI2CFunct::FindCap(uint32_t cap)
{
	return !!(m_nSemaCaps & cap);
}

uint32_t CI2CFunct::FindCapExt(uint32_t cap)
{
	return !!(m_nSemaCapsEx & cap);
}


EERROR CI2CFunct::GetVersion(char* szVersion, uint32_t nLen)
{
#if 1
	...
	if (m_clsI2CTrans.BlockTrans(m_bBMCAdr | 0x01, TT_RBL_I2C, SEMA_CMD_RD_VERSION1, \
		NULL, 0x00, (uint8_t*)szDataRet, nDataRet) == EAPI_STATUS_SUCCESS)
	{
		nDataRet2 = MAX_PATH - nDataRet;
		if (m_clsI2CTrans.BlockTrans(m_bBMCAdr | 0x01, TT_RBL_I2C, SEMA_CMD_RD_VERSION2, \
...
EERROR CI2CFunct::GetBootVersion(char* szVersion, uint32_t nLen)
{
#if 1
	EERROR eRet;
	...
	if ((eRet = m_clsI2CTrans.BlockTrans(m_bBMCAdr | 0x01, TT_RBL_I2C, SEMA_CMD_RD_BLVERSION, NULL,\
...
EERROR CI2CFunct::GetBootVersionExt(char* szVersion, uint32_t nLen)
{
#if 1
...
EERROR CI2CFunct::GetManufData(uint32_t nDataInfo, uint8_t* pData, uint32_t nLen)
{
#if 1
	...
	if (m_clsI2CTrans.BlockTrans(m_bBMCAdr | 0x01, TT_RBL_I2C, SEMA_CMD_RD_MF_DATA + nDataInfo, \
...
EERROR CI2CFunct::SetManufData(uint32_t nDataInfo, uint8_t* pData, uint32_t nLen)
{
#if 1
	...
	if(m_clsI2CTrans.BlockTrans(m_bBMCAdr, TT_WBL_I2C, SEMA_CMD_SET_ADDRESS, pData_, 3) == EAPI_STATUS_SUCCESS)
	{
		if (m_clsI2CTrans.BlockTrans(m_bBMCAdr, TT_WBL_I2C, SEMA_CMD_WRITE_DATA, to_send, 0x10) != EAPI_STATUS_SUCCESS)
		{
			return EAPI_STATUS_ERROR;
		}
...
EERROR CI2CFunct::GetTotalUpTime(uint32_t* pnTotalUpTime)
{
#if 1
	EERROR eRet;
	...
	if ((eRet = m_clsI2CTrans.BlockTrans(m_bBMCAdr | 0x01, TT_RBL_I2C, SEMA_CMD_RD_TOM, NULL, \
...
EERROR CI2CFunct::GetPwrUpTime(uint32_t* pnPwrUpTime)
{
#if 1
	EERROR eRet;
	uint8_t pDataRet[MAX_PATH]; uint32_t nDataRet = MAX_PATH;
	...
	if ((eRet = m_clsI2CTrans.BlockTrans(m_bBMCAdr | 0x01, TT_RBL_I2C, SEMA_CMD_RD_PWRUPSECS, \
...
EERROR CI2CFunct::GetPwrCycles(uint32_t* pnPwrCycle)
{
#if 1
	EERROR eRet;
	uint8_t pDataRet[MAX_PATH]; uint32_t nDataRet = MAX_PATH;
	...
	if ((eRet = m_clsI2CTrans.BlockTrans(m_bBMCAdr | 0x01, TT_RBL_I2C, SEMA_CMD_RD_PWRCYCLES, \
		NULL, 0x00, (uint8_t*)pDataRet, nDataRet)) == EAPI_STATUS_SUCCESS)
	{
		if (nDataRet == 4)
		{
...
EERROR CI2CFunct::GetBootCount(uint32_t* pnBootCount)
{
#if 1
	EERROR eRet;
	uint8_t pDataRet[MAX_PATH]; uint32_t nDataRet = MAX_PATH;
	...
	if ((eRet = m_clsI2CTrans.BlockTrans(m_bBMCAdr | 0x01, TT_RBL_I2C, SEMA_CMD_RD_BOOTCOUNT, \
		NULL, 0x00, (uint8_t*)pDataRet, nDataRet)) == EAPI_STATUS_SUCCESS)
	{
		if (nDataRet == 4)
		{
...
EERROR CI2CFunct::GetBMCFlags(uint8_t* pBMCFlag)
{
#if 1
	EERROR eRet;
	uint8_t pDataRet[MAX_PATH]; uint32_t nDataRet = MAX_PATH;
	...
	if ((eRet = m_clsI2CTrans.BlockTrans(m_bBMCAdr | 0x01, TT_RBL_I2C, SEMA_CMD_RD_BMC_FLAGS, \
		NULL, 0x00, (uint8_t*)pDataRet, nDataRet)) == EAPI_STATUS_SUCCESS)
	{
		if (nDataRet == 1)
		{
...
EERROR CI2CFunct::GetBMCStatus(uint8_t* pBMCStatus)
{
#if 1
	EERROR eRet;
	uint8_t pDataRet[MAX_PATH]; uint32_t nDataRet = MAX_PATH;
	...
	if ((eRet = m_clsI2CTrans.BlockTrans(m_bBMCAdr | 0x01, TT_RBL_I2C, SEMA_CMD_RD_BMC_STATUS, \
		NULL, 0x00, (uint8_t*)pDataRet, nDataRet)) == EAPI_STATUS_SUCCESS)
	{
		if (nDataRet == 1)
		{
...
EERROR CI2CFunct::GetRestartEvent(uint8_t* pEvent)
{
#if 1
	uint8_t pDataRet[MAX_PATH]; uint32_t nDataRet = MAX_PATH;
	...
	if (m_clsI2CTrans.BlockTrans(m_bBMCAdr | 0x01, TT_RBL_I2C, SEMA_CMD_RD_RESTARTEVT, \
		NULL, 0x00, (uint8_t*)pDataRet, nDataRet) == EAPI_STATUS_SUCCESS)
	{
		if (nDataRet == 1)
		{
...
EERROR CI2CFunct::GetRestartEventText(char* szVersion, uint32_t nSize)
{
	EERROR eRet;
	char* szRet = "";
	uint8_t bEvent = 0x00;
...
EERROR CI2CFunct::GetSysCtrlReg(uint32_t* pnCtrlReg)
{
#if 1
	EERROR eRet;
	uint8_t pDataRet[MAX_PATH]; uint32_t nRet = MAX_PATH;
	...
	*pnCtrlReg = 0x00;
	...
	if ((eRet = m_clsI2CTrans.BlockTrans(m_bBMCAdr | 0x01, TT_RBL_I2C, SEMA_CMD_SYSCFG, NULL,\
		0x00, pDataRet, nRet)) == EAPI_STATUS_SUCCESS)
	{
		switch (nRet)
		{
...
EERROR CI2CFunct::GetScaleFactor(uint8_t bChannel, uint16_t* pushScale)
{
	EERROR eRet;
	uint8_t pDataRet[MAX_PATH]; uint32_t nRet = MAX_PATH;
...
EERROR CI2CFunct::GetVolt(uint8_t bChannel, float* pfVoltage)
{
#if 1
	EERROR eRet;
	uint8_t pDataRet[MAX_PATH]; uint32_t nRet = MAX_PATH;
...
EERROR CI2CFunct::GetVoltDescEx(uint8_t bChannel, char* pData)
{
#if 1
	EERROR eRet;
	uint8_t bCount = 0x00;
	...
	if ((eRet = m_clsI2CTrans.BlockTrans(m_bBMCAdr, TT_WBL_I2C, SEMA_CMD_GET_EXT_VOLT_DESCR, \
		&bSelChannel, 0x01)) == EAPI_STATUS_SUCCESS)
	{
		while (bCount < m_nTotalChannel)
		{
			if ((eRet = m_clsI2CTrans.BlockTrans(m_bBMCAdr | 0x01, TT_RBL_I2C, SEMA_CMD_GET_EXT_VOLT_DESCR, \
				NULL, 0x00, pDataRet, nRet)) == EAPI_STATUS_SUCCESS)
			{
				if (bChannel == bCount)
				{
...
EERROR CI2CFunct::GetVoltDescExt(uint8_t bChannel, char* szDesc, uint32_t nSize, bool bTruncate)
{
#if 1
	EERROR eRet;
	uint8_t bSelChannel = 0x00;
...
EERROR CI2CFunct::GetCurrentChannel(uint8_t* pChannel)
{
#if 1
	EERROR eRet;
	uint8_t pDataRet[MAX_PATH]; uint32_t nRet = MAX_PATH;
	...
	if ((eRet = m_clsI2CTrans.BlockTrans(m_bBMCAdr | 0x01, TT_RBL_I2C, SEMA_CMD_GET_VOLT_DESCR,\
		NULL, 0x00, pDataRet, nRet)) == EAPI_STATUS_SUCCESS)
	{
		while (bStep < nRet)
		{
...
EERROR CI2CFunct::GetMainPowerCurrent(uint16_t* pushPower)
{
#if 1
	EERROR eRet;
	uint8_t pDataRet[MAX_PATH]; uint32_t nRet = MAX_PATH;
...
EERROR CI2CFunct::GetBacklightPWM(uint8_t* pPWM)
{
#if 1
	uint8_t pDataRet[MAX_PATH] = { 0 };
	uint32_t nDataRet = MAX_PATH;

	if (m_clsI2CTrans.BlockTrans(m_bBMCAdr | 0x01, TT_RBL_I2C, SEMA_CMD_BL_GETPWM, NULL,\
		0x00, (uint8_t*)pDataRet, nDataRet) == EAPI_STATUS_SUCCESS)
	{
		if (nDataRet == 1)
		{
...
EERROR CI2CFunct::SetBacklightPWM(uint8_t bPWM)
{
#if 1
	return m_clsI2CTrans.BlockTrans(m_bBMCAdr, TT_WBL_I2C, SEMA_CMD_BL_SETPWM, &bPWM, 0x01);
#endif
	return EAPI_STATUS_SUCCESS;
}

EERROR CI2CFunct::GetCurrentCPUTemp(int32_t* pfTemp)
{
#if 1
	EERROR eRet;
	...
	{
		if ((eRet = m_clsI2CTrans.BlockTrans(m_bBMCAdr | 0x01, TT_RBL_I2C, SEMA_CMD_RD_CPUTEMPCUR, \
			NULL, 0x00, pDataRet, nRet)) == EAPI_STATUS_SUCCESS)
		{
...
EERROR CI2CFunct::GetCurrentBoardTemp(int32_t* pchTemp)
{
#if 1
	EERROR eRet;
	...
	if ((eRet = m_clsI2CTrans.BlockTrans(m_bBMCAdr | 0x01, TT_RBL_I2C, SEMA_CMD_RD_BRDTEMPCUR, \
		NULL, 0x00, (uint8_t*)pDataRet, nDataRet)) == EAPI_STATUS_SUCCESS)
	{
		if (nDataRet == 1)
		{
...
EERROR CI2CFunct::GetBRDMinMaxTemp(int* pchMinBoard, int* pchMaxBoard)
{
#if 1
	EERROR eRet;
...
EERROR CI2CFunct::GetCPUMinMaxTemp(int* pchMinCPU, int* pchMaxCPU)
{
#if 1
	EERROR eRet;
	uint8_t pDataRet[MAX_PATH]; uint32_t nRet = MAX_PATH;
...
EERROR CI2CFunct::GetStartupTemp(int* pchStartBoard)
{
#if 1
	EERROR eRet;
	uint8_t pDataRet[MAX_PATH]; uint32_t nRet = MAX_PATH;
	...
	*pchStartBoard = 0x00;
	...
	if ((eRet = m_clsI2CTrans.BlockTrans(m_bBMCAdr | 0x01, TT_RBL_I2C, SEMA_CMD_RD_STARTTEMP,\
		NULL, 0x00, pDataRet, nRet)) == EAPI_STATUS_SUCCESS)
	{
		if (nRet >= 0x02)
		{
...
EERROR CI2CFunct::GetCPUStartupTemp(int* pchStartCPU)
{
#if 1
	EERROR eRet;
	uint8_t pDataRet[MAX_PATH]; uint32_t nRet = MAX_PATH;
...
EERROR CI2CFunct::GetCurrentBoardTemp_1(int* pchBoardTemp)
{
#if 1
	EERROR eRet;
	uint8_t pDataRet[MAX_PATH]; uint32_t nDataRet = MAX_PATH;
	...
	if ((eRet = m_clsI2CTrans.BlockTrans(m_bBMCAdr | 0x01, TT_RBL_I2C, SEMA_CMD_RD_2ND_BRDTEMPCUR, \
		NULL, 0x00, (uint8_t*)pDataRet, nDataRet)) == EAPI_STATUS_SUCCESS)
	{
		if (nDataRet == 1)
		{
...
EERROR CI2CFunct::GetMinMaxTemp_1(int* pchMinCPU, int* pchMaxCPU, int* pchMinBoard, int* pchMaxBoard)
{
#if 1
	EERROR eRet;
	uint8_t pDataRet[MAX_PATH]; uint32_t nRet = MAX_PATH;
...
EERROR CI2CFunct::GetStartupTemp_1(int* pchStartCPU, int* pchStartBoard)
{
#if 1
	EERROR eRet;
	uint8_t pDataRet[MAX_PATH]; uint32_t nRet = MAX_PATH;
	...
	*pchStartBoard = 0x00;
	...
	if ((eRet = m_clsI2CTrans.BlockTrans(m_bBMCAdr | 0x01, TT_RBL_I2C, SEMA_CMD_RD_2ND_STARTTEMP, \
		NULL, 0x00, pDataRet, nRet)) == EAPI_STATUS_SUCCESS)
	{
		if (nRet >= 0x02)
		{
...
EERROR CI2CFunct::SetSysCtrlReg(uint32_t nValue)
{
#if 1
	uint8_t pDataIn[] = { (uint8_t)nValue,  (uint8_t)(nValue >> 0x08),  \
		(uint8_t)(nValue >> 0x10), (uint8_t)(nValue >> 0x18) };

	return m_clsI2CTrans.BlockTrans(m_bBMCAdr, TT_WBL_I2C, SEMA_CMD_SYSCFG, pDataIn, \
		(m_nSemaCaps & SEMA_CAP_BMC_TIVA) ? 0x04 : 0x02);
#endif
	return EAPI_STATUS_SUCCESS;
}

EERROR CI2CFunct::GetErrorLog(uint32_t nPos, uint8_t* pData, uint32_t* pnDataSize, bool bSetPos)
{
#if 1
	...
	if (bSetPos)
	{
		if (m_clsI2CTrans.BlockTrans(m_bBMCAdr, TT_WBL_I2C, SEMA_CMD_GET_BOARDERRLOG, \
	...
	return m_clsI2CTrans.BlockTrans(m_bBMCAdr | 0x01, TT_RBL_I2C, SEMA_CMD_GET_BOARDERRLOG, \
		NULL, 0x00, pData, *pnDataSize);
#endif
	return EAPI_STATUS_SUCCESS;
}

EERROR CI2CFunct::GetErrorNumDescription(uint32_t ErrorNum, uint8_t* ErrorNumDes)
{
...
EERROR CI2CFunct::I2CRead(uint16_t nAdr, uint8_t cmd, uint8_t* pData, uint32_t &nSize)
{
#if 1
	return m_clsI2CTrans.BlockI2CTrans(m_bBMCAdr, TT_RBL_I2C, cmd, NULL, 0, pData, nSize);
#endif
}

EERROR CI2CFunct::I2CWrite(uint16_t nAdr, uint8_t cmd, uint8_t* pData, uint32_t nSize)
{
#if 1
	uint32_t &dummy = nSize;
	
	return m_clsI2CTrans.BlockI2CTrans(m_bBMCAdr, TT_WBL_I2C, cmd, pData, nSize, NULL, dummy);
#endif
}

EERROR CI2CFunct::GetErrorDesc(uint32_t nErrorCode, uint8_t* pData, uint32_t* pnDataSize)
{
#if 1
	uint8_t bPos[5];
	bPos[0] = (uint8_t)nErrorCode;

	if (m_clsI2CTrans.BlockTrans(m_bBMCAdr, TT_WBL_I2C, SEMA_CMD_EXC_CODE_TABLE,\
		bPos, 0x01) == EAPI_STATUS_SUCCESS)
	{
		return m_clsI2CTrans.BlockTrans(m_bBMCAdr | 0x01, TT_RBL_I2C, SEMA_CMD_EXC_CODE_TABLE,\
			NULL, 0x00, pData, *pnDataSize);
	}
#endif
	return EAPI_STATUS_ERROR;
...
EERROR CI2CFunct::GetRealChannel(char** tbString, uint8_t& bChannelRet)
{
	char szRet[MAX_PATH];
...
EERROR CI2CFunct::GetHWMonitor(char** tbString, uint32_t* pnValue)
{
	EERROR eRet;
...
EERROR CI2CFunct::GetBIOSSource(uint8_t* pIndex)
{
#if 1
	EERROR eRet;
	if (m_nSemaCaps & SEMA_CAP_FAILSAFEBIOS)
	{
		...
		if (pIndex != NULL)
		{
			uint8_t pDataRet[MAX_PATH]; uint32_t nDataRet = MAX_PATH;
			ZeroMemory(pDataRet, MAX_PATH);
			if ((eRet = m_clsI2CTrans.BlockTrans(m_bBMCAdr | 0x01, TT_RBL_I2C, SEMA_CMD_SET_SYSCTRLREG, NULL, 0x00, (uint8_t*)pDataRet, nDataRet)) == EAPI_STATUS_SUCCESS)
			{
				eRet = EAPI_STATUS_MORE_DATA;
				if (nDataRet == 1)
				{
...
EERROR CI2CFunct::SetBIOSSource(uint8_t bIndex)
{
#if 1
	...
	{
		eRet = EAPI_STATUS_ERROR;
		eRet = m_clsI2CTrans.BlockTrans(m_bBMCAdr, TT_WBL_I2C, SEMA_CMD_SET_SYSCTRLREG, &bIndex, 0x01);
	}
	return eRet;
#endif
	return EAPI_STATUS_SUCCESS;
...
EERROR CI2CFunct::GetCurrentSystemTemp(int32_t* pchTemp)
{
	return EAPI_STATUS_UNSUPPORTED;
}

EERROR CI2CFunct::GetSystemMinTemp(int32_t* pchTemp)
{
	return EAPI_STATUS_UNSUPPORTED;
}

EERROR CI2CFunct::GetSystemMaxTemp(int32_t* pchTemp)
{
	return EAPI_STATUS_UNSUPPORTED;
}

EERROR CI2CFunct::GetSystemStartupTemp(int32_t* pchTemp)
{
	return EAPI_STATUS_UNSUPPORTED;
}
