...
EERROR CI2CFunct::GetSmartFanSupport(uint32_t FanId)
{
...
EERROR CI2CFunct::GetSmartFanTempPoints(uint32_t nFanNr, char* pData, uint32_t nSize)
{
#if 1
	EERROR eRet;
	...
	if ((eRet = m_clsI2CTrans.BlockTrans(m_bBMCAdr | 0x01, TT_RBL_I2C, bCmd, NULL, \
...
EERROR CI2CFunct::GetSmartFanPWMPoints(uint32_t nFanNr, char* pData, uint32_t nSize)
{
#if 1
	EERROR eRet;
	uint8_t pDataRet[MAX_PATH]; uint32_t nRet = MAX_PATH;
...
EERROR CI2CFunct::GetSmartFanMode(uint32_t nFanNr, uint8_t* pData)
{
	EERROR eRet;
...
EERROR CI2CFunct::GetSmartFanTempSrc(uint32_t nFanNr, uint8_t* pData)
{
	EERROR eRet;
	uint32_t nCtrlReg = 0x00;
...
EERROR CI2CFunct::SetSmartFanTempPoints(uint32_t nFanNr, char* pData, uint32_t nSize)
{
#if 1
	...
	return m_clsI2CTrans.BlockTrans(m_bBMCAdr, TT_WBL_I2C, bCmd, (uint8_t*)pData, 0x04);
#endif
	return EAPI_STATUS_ERROR;
}

EERROR CI2CFunct::SetSmartFanPWMPoints(uint32_t nFanNr, char* pData, uint32_t nSize)
{
#if 1
	EERROR eRet;
	uint8_t bCmd = 0;
	...
	if (nSize >= 0x04)
	{
		eRet = m_clsI2CTrans.BlockTrans(m_bBMCAdr, TT_WBL_I2C, bCmd, (uint8_t*)pData, 0x04);
	}
	else
	{
...
EERROR CI2CFunct::SetSmartFanMode(uint32_t nFanNr, uint8_t bMode)
{
#if 1
	uint32_t nCtrlReg = 0x00;
...
EERROR CI2CFunct::SetSmartFanTempSrc(uint32_t nFanNr, uint8_t bSrc)
{
...
EERROR CI2CFunct::GetFanSpeed(uint32_t nFanNr, uint16_t* pushSpeed)
{
#if 1
