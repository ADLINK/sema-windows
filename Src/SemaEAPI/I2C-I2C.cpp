...
EERROR CI2CFunct::I2CGetBusCap(uint32_t Id, uint32_t *pMaxBlkLen)
{
...
EERROR CI2CFunct::I2CWaitForFree(void)
{
	return EAPI_STATUS_SUCCESS;
}

EERROR
CI2CFunct::I2CIsDeviceExist(uint32_t Id, uint32_t Addr)
{
#if 0
	unsigned char cmd;
	...
	tBuffer[0] = (uint8_t)Addr  << 1;
	tBuffer[1] = 0;
	tBuffer[2] = 0;
	return m_clsSemaTrans.BlockTrans(m_bBMCAdr, TT_WBL, cmd, tBuffer, 3);
#endif
	return EAPI_STATUS_SUCCESS;
}


EERROR CI2CFunct::I2CWriteReadRaw(uint32_t Id, uint32_t Addr, void *pWBuffer, uint32_t WriteBCnt, \
	void *pRBuffer, uint32_t RBufLen, uint32_t ReadBCnt)
{
	...
	if (NULL != pWBuffer)
	{
		...
		for (i = 0; i < WriteBCnt; i++) {
			tBuffer[i + 2] = ((byte*)pWBuffer)[i];
		}

		result = SemaApi_TransExt(Id, SEMA_EXT_IIC_BLOCK, WriteBCnt + 2, tBuffer);
	}
	...
	if (NULL != pRBuffer)
	{
		...
		result = SemaApi_TransExt(Id, SEMA_EXT_IIC_EXT_COMMAND_WR, 3, tBuffer);
	}
...
EERROR CI2CFunct::I2CWriteTransfer(uint32_t Id, uint32_t Addr, uint32_t Cmd, void* pBuffer, \
	uint32_t ByteCnt)
{
	unsigned int i;
	EERROR result;
...
EERROR CI2CFunct::I2CReadTransfer(uint32_t Id, uint32_t Addr, uint32_t Cmd, void* pBuffer, \
...
EERROR CI2CFunct::I2CGetStatus(uint32_t *p_output)
{
#if 0
	EERROR result;
	...
	result = m_clsSemaTrans.BlockTrans(m_bBMCAdr | 1, TT_RBL, SEMA_CMD_IIC_STATUS, NULL, 0, buf, nRet);
...
EERROR CI2CFunct::I2CTxnStatus(uint32_t* Status)
{
	return EAPI_STATUS_UNSUPPORTED;
}

EERROR CI2CFunct::I2CRawTransaction(uint32_t Id, uint32_t Addr, void* pWBuffer, uint32_t WriteBCnt, \
